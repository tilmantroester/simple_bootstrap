...
from .util import create_interpolated_cdf
...
def find_std_CI(samples, weights=None, coverage=0.683,
    ...
    if return_mean:
        result += [mean]
    if return_coverage:
        cdf = create_interpolated_cdf(samples, weights,
                                      threshold=samples.min())
        alpha = cdf(u) - cdf(l)
        result += [alpha]
...
def find_quantile_CI(samples, weights=None, coverage=0.683,
    ...
    c = (1-coverage)/2
    res = scipy.optimize.root_scalar(lambda x: cdf(x)-c,
                                     bracket=(samples_min, samples_max))
    if not res.converged:
        raise RuntimeError(f"Failed to find lower bound for tail "
                           f"coverage {c}.")
    l = res.root  # noqa: E741
    ...
    res = scipy.optimize.root_scalar(lambda x: cdf(x)-c,
                                     bracket=(samples_min, samples_max))
    if not res.converged:
        raise RuntimeError(f"Failed to find upper bound for tail "
                           f"coverage {c}.")
    u = res.root
    ...
    result = [(l, u)]
    if return_median:
        res = scipy.optimize.root_scalar(lambda x: cdf(x)-0.5,
                                         bracket=(samples_min, samples_max))
        if not res.converged:
            raise RuntimeError("Failed to find median.")
        median = res.root
        result += [median]
    if return_coverage:
        result += [alpha]
...
def find_marginal_HPDI(samples, weights=None, coverage=0.683,
    ...
    if weights is not None and weights.shape != samples.shape:
        raise ValueError(f"Samples and weights have mismatching shapes: "
                         f"{samples.shape} vs {weights.shape}.")

    samples_min = samples.min().squeeze()
    samples_max = samples.max().squeeze()

    kde = scipy.stats.gaussian_kde(samples, bw_method=kde_bandwidth,
                                   weights=weights)

    # Find maximum of marginal posterior
    res = scipy.optimize.minimize_scalar(lambda x: -kde(x),
                                         bracket=(samples_min, samples_max))
    if not res.success:
    ...
    def get_coverage(level, return_interval=False):
        ...
        if level < marg_samples_max:
            ...
            if not res.converged:
                raise RuntimeError(f"Failed to find upper bound for "
                                   f"level {level}.")
            u = res.root
    ...
    if not res.converged:
        raise RuntimeError("Failed to find coverage.")

    level = res.root
    alpha, CI = get_coverage(level, return_interval=True)

    if check_prior_edges:
        if marg_samples_min/marg_MAP_value > prior_edge_threshold \
                or marg_samples_max/marg_MAP_value > prior_edge_threshold:
            prior_edge_hit = True
        else:
...
def find_projected_joint_HPDI(samples, weights=None,
                              coverage_1d_threshold=0.683,
                              MAP=None,
                              sort_idx=None, log_posterior=None,
                              method="interpolate", twosided=True,
    ...
    return_n_sample : bool, optional
        Whether to return the number of (joint posterior) samples used to
        compute the CI.
    ...
    if sort_idx is None:
        if log_posterior is None:
            raise ValueError("If sorting indicies are not provided, the "
                             "log posterior must be given.")
    ...
    if samples[sort_idx[0]] <= samples.min():
        ...
        twosided = False
        warnings.warn("The starting point is at or lower than the sample "
                      "range. Only using one sided interpolation.")
    elif samples[sort_idx[0]] >= samples.max():
        samples[sort_idx[0]] = samples.max()
        twosided = False
        warnings.warn("The starting point is at or higher than the sample "
                      "range. Only using one sided interpolation.")
    ...
    for i in range(2, len(sort_idx)):
        (l_this, u_this), coverage_1d_this, coverage_nd_this = \
            compute_sample_range_and_coverage(samples, weights, sort_idx[:i])
        ...
        if method == "interpolate":
            if (twosided and (l_inner != l_outer and u_inner != u_outer)) \
                    or (not twosided
                        and (l_inner != l_outer or u_inner != u_outer)):
                # Got two estimates for both sides

                l_cdf_func = create_interpolated_cdf(samples, weights,
                                                     l_inner, reverse=True)
                u_cdf_func = create_interpolated_cdf(samples, weights,
                                                     u_inner, reverse=False)

                def coverage(t, l_cdf, u_cdf):
                    l = l_outer*(1-t) + l_inner*t  # noqa: E741
                ...
                def root_func(t):
                    l_cdf = l_cdf_func
                    u_cdf = u_cdf_func
                    return coverage(t, l_cdf, u_cdf) - coverage_1d_threshold

                res = scipy.optimize.root_scalar(f=root_func,
                                                 bracket=(0, 1))
                t = res.root
                l = l_outer*(1-t) + l_inner*t  # noqa: E741
                u = u_outer*(1-t) + u_inner*t
            ...
            if(l_inner != l_outer or u_inner != u_outer):
                ...
                l_idx = np.searchsorted(
                            np.cumsum(
                                weights[samples < l_inner][sample_sort_idx]),
                            delta_coverage)

                if l_idx == samples[samples < l_inner].size:
                    l_idx -= 1
                l = samples[samples < l_inner][sample_sort_idx][l_idx]  # noqa: E741, E501

                sample_sort_idx = np.argsort(samples[samples > u_inner])
                u_idx = np.searchsorted(
                            np.cumsum(
                                weights[samples > u_inner][sample_sort_idx]),
                            delta_coverage)
            ...
            if(l_inner != l_outer and u_inner != u_outer):
                ...
                res = scipy.optimize.minimize(
                            fun=fun,
                            x0=[l_inner-l_delta/2, u_inner+u_delta/2],
                            jac=fun_jac,
                            bounds=[(l_outer, l_inner), (u_inner, u_outer)],
                            constraints=[{"type": "eq",
                                          "fun": constraint,
                                          "jac": constraint_jac,
                                          "args": (l_cdf_func, u_cdf_func)}],
                            options={"ftol": 0.05},
                            method="SLSQP")
                if not res.success:
                    if res.message != \
        ...
        if strict:
            raise RuntimeError(f"Could not match 1D coverage of "
                               f"{coverage_1d_threshold}. Got coverage of "
                               f"{coverage_1d_this:.2f} for CI "
                               f"({l_this:.3f}, {u_this:.3f}).")
        else:
            warnings.warn(f"Could not match 1D coverage of "
                          f"{coverage_1d_threshold}. Got coverage of "
                          f"{coverage_1d_this:.2f} for CI "
                          f"({l_this:.3f}, {u_this:.3f}).")
            l, u = l_this, u_this
